...
const listas = ARTICLES.filter((a) => a.category === "listas-top");
...
function ListaCard({ article, index }: { article: (typeof listas)[0]; index: number }) {
  const itemCount = extractNumber(article.title);
  const isFirst = index === 0;

  return (
    <article
      className={`group cursor-pointer bg-card border overflow-hidden hover:border-primary/40 transition-all duration-200 ${
    ...
    >
      <div className={`relative overflow-hidden bg-secondary ${isFirst ? "aspect-[21/9] md:aspect-[16/5]" : "aspect-video"}`}>
        ...
        {/* Bottom overlay with title for large card */}
        {isFirst && (
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
            <TagBadge tag="LISTA" className="mb-2 block" />
            <OswaldText
              as="h2"
              className="text-2xl md:text-4xl font-bold text-white leading-tight mb-2 group-hover:text-primary transition-colors"
            >
              {article.title}
            </OswaldText>
      ...
      {/* Card body for non-featured */}
      {!isFirst && (
        <div className="p-4">
          <TagBadge tag="LISTA" className="mb-2 block" />
          <OswaldText
            as="h3"
            className="text-base font-bold text-foreground leading-snug group-hover:text-primary transition-colors mb-2"
          >
            {article.title}
          </OswaldText>
...
function ListaRowCard({ article }: { article: (typeof listas)[0] }) {
  const itemCount = extractNumber(article.title);
  return (
    <article className="group cursor-pointer flex gap-0 bg-card border border-border hover:border-primary/40 transition-colors overflow-hidden">
      ...
      <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
        <div>
          ...
          <OswaldText
            as="h3"
            className="text-base font-bold text-foreground leading-snug group-hover:text-primary transition-colors mb-1"
          >
            {article.title}
          </OswaldText>
...
export default function ListasTopPage() {
            ...
            <div className="flex items-center gap-1 flex-wrap">
              ...
              <div className="flex items-center gap-0 border border-border ml-2">
                <button
                  ...
                  className={`px-3 py-1.5 text-xs transition-colors ${viewMode === "grid" ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground"}`}
                  title="Grade"
                >
                  ⊞
                </button>
                <button
                  ...
                  className={`px-3 py-1.5 text-xs transition-colors ${viewMode === "list" ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground"}`}
                  title="Lista"
                >
                  ≡
                </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
          <div>
            ...
            {viewMode === "grid" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                {pageItems.map((a, i) => (
                  <ListaCard key={a.slug} article={a} index={i + 1} />
                ))}
              </div>
            ) : (
              <div className="space-y-3 mb-8">
                {pageItems.map((a) => (
                  <ListaRowCard key={a.slug} article={a} />
                ))}
              </div>
            )}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-6">
                <button
                  ...
                  disabled={page === 1}
                  className="p-2 border border-border text-muted-foreground hover:text-foreground hover:border-primary transition-colors disabled:opacity-30"
                >
          ...
          <aside className="space-y-8">
            ...
            <div>
              ...
              <div>
                {ARTICLES.filter((a) => a.category === "reviews").slice(0, 3).map((a) => (
